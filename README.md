...
## 밸리데이터 고민점
* 각 이벤트별로 fit하게 만들것인지, 지금처럼 범용적으로 둘것인지 고민하기.
* 현재의 방식은 쿼리가 많이나가지만, 위의 방식은 한 번의 쿼리면 끝남
* 좋은 함수를 만드는것과 성능, 두가지를 가지고 줄다리기 해야함.. 또한 유지보수에 집중하고 있기에(좋은 설계에 + 객체지향적이게) 이 점도 고려해야함

## 게이트웨이 분리 시 연결
* 페이서비스와 타 서비스들은 게이트웨이가 다른데, 어떻게 연결 할 수 있는지 고민해보기
* 정안되면 게이트웨이를 통일하는 수밖에..
